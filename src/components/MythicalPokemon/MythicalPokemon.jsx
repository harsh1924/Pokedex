...
const MythicalPokemon = () => {
  const [mythicals, setMythicals] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedPokemon, setSelectedPokemon] = useState(null);

  useEffect(() => {
    const fetchMythicalPokemon = async () => {
      const allPromises = [];

      // Loop over first 1010 Pokémon species (safe max limit)
      for (let i = 1; i <= 1010; i++) {
        allPromises.push(
          axios.get(`https://pokeapi.co/api/v2/pokemon-species/${i}`).catch(() => null)
        );
      }

      const speciesResults = await Promise.all(allPromises);
      const mythicalSpecies = speciesResults
        .filter(res => res && res.data.is_mythical)
        .map(res => res.data.id);

      // Now fetch detailed data for these Mythical Pokémon
      const details = await Promise.all(
        mythicalSpecies.map(async (id) => {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#1e103b] text-white rounded-lg overflow-hidden font-serif">
      {/* Left: List */}
      <div className="md:w-1/2 w-full bg-[#2b1a5a] p-6 overflow-y-auto scrollbar-hide max-h-[90vh]">
        <h1 className="text-4xl font-bold mb-6">Mythical Pokémon</h1>
        <ul className="space-y-2">
          {mythicals?.map((p, index) => (
            <li
              key={p.id}
