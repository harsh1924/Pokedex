...
const paradoxPokemonNames = [
    "great-tusk", "scream-tail", "brute-bonnet", "flutter-mane",
    "slither-wing", "sandy-shocks", "roaring-moon", "iron-treads",
    "iron-bundle", "iron-hands", "iron-jugulis", "iron-moth",
    "iron-thorns", "iron-valiant", "walking-wake", "iron-leaves",
...
export default function ParadoxPokemon() {
    const [pokemonList, setPokemonList] = useState([]);
    const [selected, setSelected] = useState(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        async function fetchParadox() {
            const result = await Promise.all(
                paradoxPokemonNames.map(async (name) => {
    ...
    return (
        <div className="flex flex-col md:flex-row bg-[#1e103b] text-white rounded-lg overflow-hidden font-serif">
            {/* Left List */}
            <div className="md:w-1/2 w-full bg-[#2b1a5a] scrollbar-hide p-6 max-h-[90vh] overflow-y-auto">
                <h1 className="text-4xl font-bold mb-6">Paradox Pokémon</h1>
                    ...
                    {pokemonList.map((p, index) => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="capitalize font-bold">{p.name.replace("-", " ")}</span>
                            </div>
