  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await Promise.all(
        pseudoLegendaryList.map(async ({ name }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#1e103b] text-white rounded-lg overflow-hidden font-serif">
      ...
      <div className="md:w-1/2 w-full bg-[#2b1a5a] p-6 overflow-y-auto max-h-[90vh]">
        <h1 className="text-4xl font-bold mb-6">Pseudo-Legendary Pokémon</h1>
      ...
      <div className="md:w-1/2 w-full bg-[#1e103b] px-6 pt-10 pb-6 flex flex-col relative gap-60">
        {selected && (
          <>
            ...
            />
            <div>
              <h2 className="text-6xl tracking-widest font-bold uppercase">{selected.name}</h2>
              <p className="text-gray-300 text-sm mb-30">#{selected.id}</p>
            </div>
...
export default PseudoLegendaryPokemon;