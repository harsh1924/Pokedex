...
const LegendaryPokemon = () => {
  ...
  useEffect(() => {
    const fetchLegendaryPokemon = async () => {
      ...
      const speciesResults = await Promise.all(allSpecies);
      const legendaryIds = speciesResults
        .filter(res => res && res.data.is_legendary)
        .map(res => res.data.id);

      const details = await Promise.all(
        legendaryIds.map(async (id) => {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#1e103b] text-white rounded-lg overflow-hidden font-serif">
      ...
      <div className="md:w-1/2 w-full bg-[#2b1a5a] p-6 overflow-y-auto scrollbar-hide max-h-[90vh]">
        <h1 className="text-4xl font-bold mb-6">Legendary Pokémon</h1>
        <ul className="space-y-2">
          {legendaries?.map((p, index) => (
            <li
              key={p.id}
